...
class ClientUI():
    ...
    def configure_login(self):
        ...
        self.savePassCB = ttk.Checkbutton(
            self.master, text="Remember Me", variable=self.savePass
        )
    ...
    def insert(self, message, title=None):
        ...
        if self.master.focus_get() == None and self.notifications.get() == 1 and not self.notified:
            ...
            if whisper or error:
                ...
                label.config(fg=self.whispercolor if whisper else self.errorcolor)
            else:
                label = tk.Label(
    ...
    def register(self, event=None):
        ...
        self.entries.append(ttk.Entry(
            self.master, width=self.entries[0]["width"], show=self.entries[2]["show"], font=self.font))
    ...
    def configure_chatroom(self):
        ...
        self.helpMenu.add_command(label='API Documentation', command=lambda: self.text_window(
        ...
        self.statsMenu.add_command(
            label='Leaderboard', command=self.createLeaderboard)
        ...
        self.insert('Server> This is a private place to take notes. ',
        ...
        ttk.Label(self.master, text=self.master.username +
    ...
    def configure_font(self):
        ...
        self.spinbox = Spinbox(self.cfwin, width=10,
                               from_=1, to_=72, wrap=True, font=self.sFont)
    ...
    def ip_info(self):
        ...
        label['text'] = 'Local IP: ' + self.master.localIP + ', Port: ' + str(self.master.port) + \
                        '\nServer IP: ' + \
            serverInfo[0] + ', Port: ' + str(serverInfo[1])
    ...
    def tac_info(self):
        ...
        agree = tk.BooleanVar()
        agree.set(False)
        tk.Label(self.master, bg=self.bg, text='I made it short for you. Please read it. ',
                 font=self.bFont).grid(row=1, column=0, pady=5)
        checkButton = ttk.Checkbutton(
            self.master, text='I agree to these Terms and Conditions. ', variable=agree)
        nextButton = ttk.Button(self.master, text='Continue',
                                state='disabled', command=lambda event=None: self.accept_TaC())
        checkButton.config(
            command=lambda: self.nextButtonState(nextButton, agree))
    ...
    def createLeaderboard(self):
        ...
        updateButton = ttk.Button(self.leaderboard, width=19, text='Update Leaderboard',
    ...
    def del_account(self):
        ...
        self.daTitle = tk.Label(
            self.dawin, text='To confirm deletion, please enter your password below. ', font=self.font, fg=self.fg, bg=self.bg)
